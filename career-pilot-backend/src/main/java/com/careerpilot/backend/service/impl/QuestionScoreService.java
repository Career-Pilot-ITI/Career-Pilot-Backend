 ...
 *
 * Pipeline:
 * SessionQuestion (transcript + pre-computed pacing stored by
 * SessionQuestionService)
 * │
 * ├─ Read pacing score from sq.speechRateWpm (already computed + saved — no
 * re-parsing)
 * │
 * └─→ LlmService.scoreAnswer(questionId, userId, transcript)
 * → {contentRelevance, clarity, confidence, fillerWords}
 * │
 * └─ overall = content(40%) + clarity(20%) + confidence(20%) + pacing(10%) +
 * filler(10%)
 */
@Service
...
public class QuestionScoreService implements IQuestionScoreService {
  ...
  public QuestionScoreResponse scoreAnswer(SessionQuestion sessionQuestion) {
    ...
    // 1. Compute pacing score from the already-saved speechRateWpm field (no
    // re-parsing)
    ...
    // Generate and save per-question coaching tip
    try {
      String tip = llmService.generateQuestionTip(
          sessionQuestion.getQuestionText(),
          sessionQuestion.getUserTranscript(),
          saved.getContentRelevance(), saved.getClarity(),
          saved.getConfidence(), saved.getPacing(), saved.getFillerWords());
      saved.setCoachingTip(tip);
      scoreRepository.save(saved);
    } catch (Exception e) {
      log.error("Failed to generate coaching tip for question {}: {}", sessionQuestion.getId(), e.getMessage(), e);
    }
  ...
  /**
   * Compute pacing score 0-100 from the pre-computed speechRateWpm saved on
   * SessionQuestion.
   * SessionQuestionService already did the arithmetic — we simply read the field
   * here.
   *
